...
extern char pmic_string[2][PMIC_STRING_MAXLENGTH];
...
#endif
	ADD_STR_TO_VARMON(pmic_string[0]);
	ADD_STR_TO_VARMON(pmic_string[1]);
...
int summary_set_task_info(struct sec_debug_summary_data_apss *apss)
{
	...
	SET_MEMBER_TYPE_INFO(&apss->task.ts.thread_group_next, struct task_struct, thread_group.next);
	SET_MEMBER_TYPE_INFO(&apss->task.ts.fp, struct task_struct, thread.cpu_context.fp);
...
int __init sec_debug_summary_init(void)
{
	...
	/* logger.c file was not support in DREAM 

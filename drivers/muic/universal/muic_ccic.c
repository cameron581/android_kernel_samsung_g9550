...
int muic_is_ccic_supported_dev(muic_data_t *pmuic, muic_attached_dev_t new_dev)
{
	...
	if (pmuic->vps_table == VPS_TYPE_SCATTERED) {
		if(pmuic->vps.s.adc != ADC_OPEN) {
			...
			pmuic->is_pba_array = true;
			return 1;
		}
	}
	else if (pmuic->vps.t.adc != ADC_OPEN) {
		pr_info("%s: Detect Legacy Cable in PBA array(0x%x)\n",
				__func__, pmuic->vps.t.adc);
		pmuic->is_pba_array = true;
		return 1;
	}
#endif
...
#endif
	if (pmuic->vps_table == VPS_TYPE_SCATTERED && get_switch_mode(pmuic) != SWMODE_AUTO)
...
int mdev_continue_for_TA_USB(muic_data_t *pmuic, int mdev)
{
	...
	if ((pmuic->attached_dev == ATTACHED_DEV_JIG_UART_OFF_MUIC || pmuic->attached_dev == ATTACHED_DEV_JIG_UART_OFF_VB_OTG_MUIC)
			...
			&& pmuic->is_pba_array) {
		muic_send_dock_intent(MUIC_DOCK_DESKDOCK);
		msleep(500);
...
static int mdev_handle_legacy_TA_USB(muic_data_t *pmuic)
{
	struct mdev_desc_t *pdesc = &mdev_desc;
	int mdev = 0, vbvolt;
	(pmuic->vps_table == VPS_TYPE_SCATTERED) ? (vbvolt = pmuic->vps.s.vbvolt) : (vbvolt = pmuic->vps.t.vbvolt);

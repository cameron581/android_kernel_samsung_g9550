...
u16 device_driver_mode;		/* = FC8350_TUNER*/;
u16 g_pkt_length;
...
int data_callback(void *hDevice, u8 bufid, u8 *data, int len)
{
	...
	list_for_each(temp, &(hInit->hHead))
	{
		...
		if (hOpen->isdbttype == TS_TYPE) {
...
#ifdef TS_DROP_DEBUG
				print_log(NULL, "RingBuffer full\n");
#endif
				/* return 0 */;
				FCI_RINGBUFFER_SKIP(&hOpen->RingBuffer, len);
			}
#ifdef TS_DROP_DEBUG
			print_log(NULL, "RingBuffer Write %d\n", len);
#endif
			...
			print_log(NULL, "Data callback : TS Stop\n");
#endif
	}
...
int isdbt_hw_setting(HANDLE hDevice)
{
	...
	err = request_threaded_irq(gpio_to_irq(isdbt_pdata->gpio_int), isdbt_irq
		...
		, FC8300_NAME, hInit);
	if (err < 0) {
		print_log(0, "couldn't request interrupt %d reason(%d)\n"
...
void isdbt_hw_init(void)
{
	...
	pr_err("%s, gpio_en =%d  gpio_rst = %d\n", __func__
		...
		, gpio_get_value(isdbt_pdata->gpio_rst));
	...
	mutex_unlock(&power_onoff_lock);
#ifdef BBM_SPI_IF
	if (device_driver_mode == FC8350_TUNER)
...
ssize_t isdbt_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	if (!(read_len%188)) {
		...
		print_log(NULL, "Read Len Error %d\n", read_len);
#endif
#endif

#ifdef TS_DROP_DEBUG
	print_log(hInit, "RingBuffer Read %d  Buffer : %d\n", read_len, count);
#endif
	return read_len;
}
...
{
	s32 res;
	u16 addr = 0x26, chipid;

	isdbt_hw_init();

	/* check if FC8300? */
	res = bbm_com_word_read(hInit, DIV_BROADCAST, addr, &chipid);
	if (res) {
		print_log(hInit, "%s reading 8300 chip id err %d\n"
			, __func__, res);
		goto exit;
	}
	if (chipid == 0x8300) {
		device_driver_mode = FC8300_TUNER;
		print_log(hInit, "%s reg %#x id %#x\n", __func__, addr, chipid);
		goto exit;
	}

	/* check if FC8350? */
#ifdef BBM_SPI_IF
	res = bbm_com_byte_write(hInit, DIV_BROADCAST
				, BBM_DM_DATA, 0x00);
#endif
	res |= bbm_com_word_read(hInit, DIV_BROADCAST, addr, &chipid);
	if (res) {
		print_log(hInit, "%s reading 8350 chip id err %d\n"
			, __func__, res);
		goto exit;
	}
	if (chipid == 0x8350) {
		device_driver_mode = FC8350_TUNER;
		print_log(hInit, "%s reg %#x id %#x\n", __func__, addr, chipid);
		goto exit;
	}

	print_log(hInit, "%s wrong all chip id %#x\n"
			, __func__, chipid);
	res = BBM_NOK;
...
long isdbt_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_ISDBT_INIT:
		pr_err("[FC8300] IOCTL_ISDBT_INIT, tuner %d\n", device_driver_mode);
		...
		pr_err("[FC8300] ISDBT_INIT BBM : %s Xtal : %d, DEV : %s\n"
	...
	case IOCTL_ISDBT_BULK_READ:
		...
		if (info.buff[1] >
			...
			print_log(hInit, "BULK_READ sizeErr %d\n"
				, info.buff[1]);
			res = BBM_NOK;
			break;
		}
	...
	case IOCTL_ISDBT_BULK_WRITE:
		...
		if (info.buff[1] >
			(sizeof(info.buff) - sizeof(info.buff[0]) * 2)) {
			print_log(hInit, "BULK_WRITE sizeErr %d\n"
				, info.buff[1]);
			res = BBM_NOK;
			break;
		}
	...
	case IOCTL_ISDBT_TUNER_SELECT_2:
		...
		if (((u32)info.buff[1] == ISDBTMM_13SEG)
			...
			pm_qos_add_request(&fc8300_cpu_handle
			...
			print_log(hInit, "TUNER THRESHOLD: %d\n"
		...
		print_log(hInit, "IOCTL_ISDBT_TUNER_SELECT_2 %d\n"
			, (u32)info.buff[1]);
		break;
	...
	case IOCTL_ISDBT_TUNER_SELECT:
		...
		err = copy_from_user((void *)&info, (void *)arg, size);
		info.buff[0] = device_driver_mode;
		...
		print_log(hInit, "IOCTL_ISDBT_TUNER_SELECT %d\n"
		, (u32)info.buff[1]);
		break;
	...
	case IOCTL_ISDBT_TUNER_PKT_MODE:
		...
		if (!err) {
			...
			print_log(hInit, "IOCTL_ISDBT_TUNER_PKT_MODE %lu\n"
			, info.buff[0]);
		}
		res = err;
		break;
...
static struct isdbt_platform_data *isdbt_populate_dt_pdata(struct device *dev)
{
	...
	pdata->gpio_en = of_get_named_gpio(dev->of_node
	...
	pdata->gpio_rst = of_get_named_gpio(dev->of_node
											, "isdbt,isdb-gpio-rst", 0);
	if (pdata->gpio_rst < 0)
		of_property_read_u32(dev->of_node
			, "isdbt,isdb-gpio-rst", &pdata->gpio_rst);
	if (pdata->gpio_rst < 0) {
	...
	pdata->gpio_lna_en = of_get_named_gpio(dev->of_node
								, "isdbt,isdb-gpio-lna-en", 0);
	if (pdata->gpio_lna_en < 0)
		of_property_read_u32(dev->of_node
			, "isdbt,isdb-gpio-lna-en", &pdata->gpio_lna_en);
	if (pdata->gpio_lna_en < 0) {
...
*/
	...
	pdata->gpio_int = of_get_named_gpio(dev->of_node
							, "isdbt,isdb-gpio-irq", 0);
	if (pdata->gpio_int < 0)
		of_property_read_u32(dev->of_node
			, "isdbt,isdb-gpio-irq", &pdata->gpio_int);
	if (pdata->gpio_int < 0) {
	...
	ret = of_property_read_string(dev->of_node
				, "isdbt,tuner_id", &temp_string);
	if (ret) {
		pr_err("%s: cannot get tuner_id(%d)", __func__, ret);
	} else {
		pr_info("%s: tuner_id(%s)\n", __func__, temp_string);
		if (!strncmp(temp_string, DT_FC8300, strlen(DT_FC8300)))
			device_driver_mode = FC8300_TUNER;
		else if (!strncmp(temp_string, DT_FC8350, strlen(DT_FC8350)))
			device_driver_mode = FC8350_TUNER;
		else {
			pr_err("%s: unknown tuner_id\n", __func__);
			device_driver_mode = -1;
		}
		/*pr_info("%s: device_driver_mode(%u)\n", __func__, device_driver_mode);*/
	}
	...
	if (pdata->isdbt_clk < 0)
		pr_err("%s: cannot get clock", __func__);
...
static bool isdbt_is_valid_tuner_id(struct isdbt_platform_data *pdata)
{
	if ((device_driver_mode == FC8300_TUNER) ||
		(device_driver_mode == FC8350_TUNER))
		return true;
	else
		return false;
...
static int isdbt_probe(struct platform_device *pdev)
{
	...
	if (!isdbt_is_valid_tuner_id(isdbt_pdata)) {
		res |= isdbt_chip_id();
		if (res)

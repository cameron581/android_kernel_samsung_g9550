...
static int msm_ois_init(struct msm_ois_ctrl_t *a_ctrl) {
    ...
    if (!a_ctrl->is_shift_enabled && a_ctrl->shift_tbl.ois_shift_used) {
...
static int32_t msm_ois_read_user_data_section(struct msm_ois_ctrl_t *a_ctrl, uint16_t addr, int size, uint8_t *user_data)
{
    ...
    memset(&a_ctrl->shift_tbl, 0, sizeof(a_ctrl->shift_tbl));
...
int msm_ois_create_shift_table(struct msm_ois_ctrl_t *a_ctrl, uint8_t *shift_data){
    ...
    if (shift_data[0] != 0x11){
        a_ctrl->shift_tbl.ois_shift_used = false;
        return 0;
    }
    a_ctrl->shift_tbl.ois_shift_used = true;
...
static int32_t msm_ois_power_down(struct msm_ois_ctrl_t *a_ctrl)
{
    ...
    if (a_ctrl->ois_state != OIS_POWER_DOWN) {
        ...
        rc = msm_cam_clk_enable(&(a_ctrl->i2c_client.client->dev),
                                    a_ctrl->clk_info,
                                    a_ctrl->ois_clk,
                                    a_ctrl->num_clks,
                                    0);
    ...
    const struct i2c_device_id *id)
{
    ...
    rc = msm_camera_i2c_dev_get_clk_info(&client->dev, &ois_ctrl_t->clk_info,
            &ois_ctrl_t->ois_clk, &ois_ctrl_t->num_clks);
    if (rc < 0) {
...
static int32_t msm_ois_platform_probe(struct platform_device *pdev)
{
    ...
    rc = msm_camera_get_clk_info(pdev, &msm_ois_t->clk_info,
            &msm_ois_t->ois_clk, &msm_ois_t->num_clks);
    if (rc < 0) {
        pr_err("%s: failed to get the clocks\n", __func__);
    }

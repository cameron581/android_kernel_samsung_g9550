...
static void sending_tune_cmd(struct device *dev, char *src, int len)
{
	...
	if (IS_ERR_OR_NULL(vdd))
		...
		if ((vdd->ctrl_dsi[DSI_CTRL_0]->cmd_sync_wait_broadcast)
			...
			vdd->dtsi_data[DSI_CTRL_1].panel_tx_cmd_list[TX_MDNIE_TUNE][vdd->panel_revision].cmd_cnt = 3;
			...
			vdd->dtsi_data[DSI_CTRL_0].panel_tx_cmd_list[TX_MDNIE_TUNE][vdd->panel_revision].cmd_cnt = vdd->mdnie_tuning_enable_tft ? 6 : 3;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (vdd->copr.copr_on) {
		...
		vdd->copr.copr_avr = vdd->copr.copr_sum / vdd->copr.total_t;
		vdd->copr.cd_avr = vdd->copr.cd_sum / vdd->copr.total_t;

...
void mdss_samsung_dump_regs(void)
{
	...
	if (mdata->nvig_pipes) {
		...
		for (loop = 0; loop < mdata->nvig_pipes ; loop++) {
			snprintf(name, sizeof(name), "VG%d_SSPP", loop);
			LCD_ERR("=============%s 0x%08zx ==============\n", name,
				kvaddr_to_paddr((unsigned long)mdata->vig_pipes[loop * max_rects].base));
			dump_reg(mdata->vig_pipes[loop].base, 0x200);

			snprintf(name, sizeof(name), "VG%d", loop);
			LCD_ERR("=============%s 0x%08zx ==============\n", name,
				kvaddr_to_paddr((unsigned long)mdata->vig_pipes[loop * max_rects].base) + 0xa00);
			dump_reg(mdata->vig_pipes[loop].base+0xa00, 0x100);
		}
	}
	...
	if (mdata->ndma_pipes) {
		...
		for (loop = 0; loop < mdata->ndma_pipes ; loop++) {
			...
			dump_reg(mdata->dma_pipes[loop * max_rects].base, 0x400);
		}
	}
	...
	list_for_each_entry(pipe, &mdp5_data->pipes_used, list) {
		if (pipe && pipe->src_fmt)
			LCD_ERR("[%4d, %4d, %4d, %4d] -> [%4d, %4d, %4d, %4d]"
				"|flags = %16llx|src_format = %2d|bpp = %2d|ndx = %3d|\n",
	...
	*/
	...
	for (z_order = MDSS_MDP_STAGE_0; z_order < MDSS_MDP_MAX_STAGE; z_order++) {
		list_for_each_entry(pipe, &mdp5_data->pipes_used, list) {
			...
			LCD_ERR("L_mixer:%d R_mixer:%d %s z:%d format:%d %s flag:0x%llx src.x:%d y:%d w:%d h:%d "
...
static void mdss_sasmung_panel_debug_create(struct samsung_display_driver_data *vdd)
{
	...
	if (!IS_ERR_OR_NULL(debug_data->is_factory_mode))
		debugfs_create_bool("is_factory_mode", 0600, debug_data->root,	debug_data->is_factory_mode);

...
enum mipi_samsung_tx_cmd_list {
	...
	TX_ACT_CLOCK_ENABLE,
	TX_ACT_CLOCK_DISABLE,
	TX_ACT_CLOCK_IMG_DATA,
	TX_HBM_GAMMA,
	TX_HBM_ETC,
	TX_HBM_IRC,
	TX_HBM_OFF,
...
struct ACT_CLOCK {
	bool is_support;
	bool img_need_update;
	bool update_image;
	u32 file_opend;
	struct miscdevice dev;
	struct act_clk_info clk_info;
	struct act_blink_info blink_info;
	struct act_drawer_info draw_info;
	struct dsi_panel_cmds act_clock_control_tx_cmds;
	struct dsi_panel_cmds act_clock_disable_tx_cmds;
	struct dsi_panel_cmds act_clock_img_data_tx_cmds;
};

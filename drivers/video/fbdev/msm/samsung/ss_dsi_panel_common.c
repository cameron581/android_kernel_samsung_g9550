			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	*/
	...
	for (loop = 0; loop < SUPPORT_PANEL_COUNT; loop++) {
		...
		vdd_data.vdd_blank_mode_done[loop] = FB_BLANK_POWERDOWN;
	}
	...
	u32 *flags)
{
	...
	if (pinfo->blank_state == MDSS_PANEL_BLANK_LOW_POWER) {
		switch (cmd) {
			...
			case TX_MULTIRES_HD:
			case TX_ACT_CLOCK_ENABLE:
			case TX_ACT_CLOCK_DISABLE:
			case TX_ACT_CLOCK_IMG_DATA:
			case TX_MDNIE_TUNE:
				break;
			default:
	...
	switch (cmd) {
		case TX_ACT_CLOCK_ENABLE:
			cmds = &vdd->act_clock.act_clock_control_tx_cmds;
			break;
		case TX_ACT_CLOCK_DISABLE :
			cmds = &vdd->act_clock.act_clock_disable_tx_cmds;
			break;
		case TX_ACT_CLOCK_IMG_DATA :
			cmds = &vdd->act_clock.act_clock_img_data_tx_cmds;
			break;
		default:

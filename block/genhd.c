			     ...
			     char *buf)
{
	...
	return sprintf(buf,"\"ReadC\":\"%lu\",\"ReadKB\":\"%lu\","
...
static ssize_t disk_ios_store(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{
	...
	disk->accios.ios[READ] = part_stat_read(hd, ios[READ]);
	disk->accios.ios[WRITE] = part_stat_read(hd, ios[WRITE]);
	disk->accios.ios[DISCARD] = part_stat_read(hd, discard_ios);
	disk->accios.sectors[READ] = part_stat_read(hd, sectors[READ]);
	disk->accios.sectors[WRITE] = part_stat_read(hd, sectors[WRITE]);
	disk->accios.sectors[DISCARD] = part_stat_read(hd, discard_sectors);
	get_monotonic_boottime(&(disk->accios.uptime));
...
static DEVICE_ATTR(diskios, S_IRUSR|S_IWUSR, disk_ios_show, disk_ios_store);

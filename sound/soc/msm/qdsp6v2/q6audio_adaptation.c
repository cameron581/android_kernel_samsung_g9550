...
/****************************************************************************/
static int32_t q6audio_adaptation_dsm_callback(struct apr_client_data *data, void *priv)
{
	u32 param_id;
	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2) {
		...
		pr_info("%s: param_id=%#x payload_size=%d\n",
						__func__, param_id, data->payload_size);
		...
		if (param_id == AFE_PARAM_ID_CALIB_RES_CFG) {
			...
			if (data->payload_size < sizeof(this_afe.calib_log_data)) {
			...
			if (!this_afe.calib_log_data.status) {
				...
				pr_debug("%s: calib log resp status: %d", __func__,
...
static inline void q6asm_set_flag_in_token_in_adaptation(union asm_token_struct *asm_token,
...
static inline int q6asm_get_flag_from_token_in_adaptation(union asm_token_struct *asm_token,
					    int flag_offset)
{
...
static inline void q6asm_update_token_in_adaptation(u32 *token, u8 session_id, u8 stream_id,
				      u8 buf_index, u8 dir, u8 nowait_flag)
{
	...
	q6asm_set_flag_in_token_in_adaptation(&asm_token, dir, ASM_DIRECTION_OFFSET);
	q6asm_set_flag_in_token_in_adaptation(&asm_token, nowait_flag,
				  ASM_CMD_NO_WAIT_OFFSET);
	*token = asm_token.token;
}

/* This function must be sync up from __q6asm_add_hdr_async() of q6asm.c */
static void __q6asm_add_hdr_async_in_adaptation(struct audio_client *ac, struct apr_hdr *hdr,
...
static void q6asm_add_hdr_async_in_adaptation(struct audio_client *ac, struct apr_hdr *hdr,
				uint32_t pkt_size, uint32_t cmd_flg)
{
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = q6asm_set_sound_alive(ac, (long *)ucontrol->value.integer.value);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	msm_pcm_routing_get_fedai_info(SEC_ADAPTATAION_AUDIO_PORT,
			SESSION_TYPE_RX, &fe_dai_map);
	ac = q6asm_get_audio_client(fe_dai_map.strm_id);
	ret = q6asm_set_play_speed(ac, (long *)ucontrol->value.integer.value);
	mutex_unlock(&asm_lock);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	msm_pcm_routing_get_fedai_info(SEC_ADAPTATAION_AUDIO_PORT,
			SESSION_TYPE_RX, &fe_dai_map);
	ac = q6asm_get_audio_client(fe_dai_map.strm_id);
	ret = q6asm_set_adaptation_sound(ac, (long *)ucontrol->value.integer.value);
	mutex_unlock(&asm_lock);
	return ret;
}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	msm_pcm_routing_get_fedai_info(SEC_ADAPTATAION_AUDIO_PORT,
			SESSION_TYPE_RX, &fe_dai_map);
	ac = q6asm_get_audio_client(fe_dai_map.strm_id);
	ret = q6asm_set_sound_balance(ac, (long *)ucontrol->value.integer.value);
	mutex_unlock(&asm_lock);
	return ret;
}
...
/* This function must be sync up from voice_itr_get_next_session() of q6voice.c */
...
/* This function must be sync up from is_source_tracking_shared_memomry_allocated() of q6voice.c */
...
static int32_t q6audio_adaptation_cvs_callback(struct apr_client_data *data, void *priv)
{
	int i;
	uint32_t *ptr = NULL;
...
static int send_packet_loopback_cmd(struct voice_data *v, int mode)
{
	...
	if (this_cvs.apr == NULL) {
		this_cvs.apr = apr_register("ADSP", "CVS",
					...
					SEC_ADAPTATAION_LOOPBACK_SRC_PORT, &this_cvs);
	}
...
static int32_t q6audio_adaptation_cvp_callback(struct apr_client_data *data, void *priv)
{
	int i;
	uint32_t *ptr = NULL;
...
{
	struct cvp_adaptation_sound_parm_send_cmd cvp_adaptation_sound_param_cmd;
	int ret = 0;
	u16 cvp_handle;
	...
	if (this_cvp.apr == NULL) {
		this_cvp.apr = apr_register("ADSP", "CVP",
					...
					SEC_ADAPTATAION_VOICE_SRC_PORT, &this_cvp);
	}
	cvp_handle = voice_get_cvp_handle(v);

	/* fill in the header */
	cvp_adaptation_sound_param_cmd.hdr.hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
	...
	cvp_adaptation_sound_param_cmd.adaptation_sound_data.module_id = VOICE_VOICEMODE_MODULE;
	cvp_adaptation_sound_param_cmd.adaptation_sound_data.param_id = VOICE_ADAPTATION_SOUND_PARAM;
	...
	memcpy(cvp_adaptation_sound_param_cmd.adaptation_sound_data.param, parameters, sizeof(int16_t)*12);
	...
	ret = apr_send_pkt(this_cvp.apr, (uint32_t *) &cvp_adaptation_sound_param_cmd);
	if (ret < 0) {
...
int sec_voice_set_adaptation_sound(uint16_t mode, uint16_t select, int16_t *parameters)
{
	struct voice_data *v = NULL;
	int ret = 0;
	struct voice_session_itr itr;
	...
	while (voice_itr_get_next_session(&itr, &v)) {
		if (v != NULL) {
			...
			if (is_voc_state_active(v->voc_state) &&
				...
				!v->disable_topology)
				ret = sec_voice_send_adaptation_sound_cmd(v, mode, select, parameters);
			mutex_unlock(&v->lock);
		} else {
			pr_err("%s: invalid session\n", __func__);
			ret = -EINVAL;
...
static int sec_voice_send_nb_mode_cmd(struct voice_data *v, int enable)
{
	...
	cvp_nbmode_cmd.hdr.src_port = SEC_ADAPTATAION_VOICE_SRC_PORT;
...
static int sec_voice_send_device_info_cmd(struct voice_data *v, int device)
{
	...
	cvp_device_info_cmd.hdr.opcode = VSS_ICOMMON_CMD_SET_UI_PROPERTY;
	cvp_device_info_cmd.cvp_set_device_info.module_id = VOICE_MODULE_SET_DEVICE;
	cvp_device_info_cmd.cvp_set_device_info.param_id = VOICE_MODULE_SET_DEVICE_PARAM;
	cvp_device_info_cmd.cvp_set_device_info.param_size = MOD_ENABLE_PARAM_LEN;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < 12; i++) {
		adaptation_sound_param[i] = (short)ucontrol->value.integer.value[2+i];
	...
	return sec_voice_set_adaptation_sound(adaptation_sound_mode,
				adaptation_sound_select, adaptation_sound_param);
}
...
static const struct snd_kcontrol_new samsung_solution_mixer_controls[] = {
	SOC_SINGLE_MULTI_EXT("SA data", SND_SOC_NOPM, 0, 65535, 0, 27,
				sec_audio_sound_alive_get, sec_audio_sound_alive_put),
	SOC_SINGLE_MULTI_EXT("VSP data", SND_SOC_NOPM, 0, 65535, 0, 1,
				sec_audio_play_speed_get, sec_audio_play_speed_put),
	SOC_SINGLE_MULTI_EXT("Audio DHA data", SND_SOC_NOPM, 0, 65535, 0, 14,
				sec_audio_adaptation_sound_get, sec_audio_adaptation_sound_put),
	SOC_SINGLE_MULTI_EXT("LRSM data", SND_SOC_NOPM, 0, 65535, 0, 2,
				sec_audio_sound_balance_get, sec_audio_sound_balance_put),
	...
	SOC_SINGLE_MULTI_EXT("SB enable", SND_SOC_NOPM, 0, 65535, 0, 1,
				sec_audio_sound_boost_get, sec_audio_sound_boost_put),
	...
	SOC_SINGLE_MULTI_EXT("Sec Set DHA data", SND_SOC_NOPM, 0, 65535, 0, 14,
				sec_voice_adaptation_sound_get, sec_voice_adaptation_sound_put),
